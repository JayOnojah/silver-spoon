...
export default auth((req): any => {
  ...
  if (isLoggedIn && nextUrl.pathname === DEFAULT_LOGIN_REDIRECT) {
    ...
    if (user?.defaultTenantType === "hotel") {
      return NextResponse.redirect(new URL("/dashboard/hotel", nextUrl));
    } else {
      return NextResponse.redirect(new URL("/dashboard/restaurant", nextUrl));
    }
  }
  ...
  if (isLoggedIn) {
    ...
    if (user?.defaultTenantId) {
      ...
      response.cookies.set("tenant_id", user.defaultTenantId, {
        path: "/",
