...
export const MoodboardItems = () => {
  ...
  const designs: MoodboardCardProps[] = [
  ...
  // ✅ debounce (use debouncedQuery in filtering)
  useEffect(() => {
    const timer = setTimeout(() => setDebouncedQuery(searchQuery), 300);
    return () => clearTimeout(timer);
  }, [searchQuery]);

  // ✅ counts (keeps tabs accurate)
  const tabs: TabItem[] = useMemo(() => {
    const allCount = designs.length;
    const ordersCount = designs.filter((d) => d.owner.toLowerCase().includes("order")).length;
    const customersCount = designs.filter((d) => {
      const o = d.owner.toLowerCase();
      return o.includes("customer") || (!o.includes("order") && !o.includes("personal"));
    }).length;
    const personalCount = designs.filter((d) => d.owner.toLowerCase().includes("personal")).length;

    return [
      { id: "all", label: "All Moodboard", count: allCount },
      { id: "orders", label: "For Orders", count: ordersCount },
      { id: "customers", label: "For Customers", count: customersCount },
      { id: "personal", label: "Personal" }, // personal have no count
      // If personal is to count: { id: "personal", label: "Personal", count: personalCount }
    ];
  }, [designs]); 
  ...
  const filteredDesigns = useMemo(() => {
    ...
    if (activeTab === "orders") {
      ...
      filtered = filtered.filter((d) => d.owner.toLowerCase().includes("personal"));
    }

    // ✅ search filter uses debouncedQuery
    ...
    return filtered;
  }, [activeTab, debouncedQuery, designs]);

  return (
    <div className="min-h-screen bg-[#FFF1EC] pt-5 font-sans">
      <div>
        {/* ✅ Tabs */}
        <MoodboardTabs tabs={tabs} activeTab={activeTab} setActiveTab={setActiveTab} />
            ...
            <PaginationContent className="gap-5">
              <PaginationItem className="border rounded-full h-10 w-10 flex justify-center items-center border-[#CDD5DF]">
                <PaginationPrevious
                  ...
                  className={`text-[#121926] hover:bg-transparent hover:-translate-x-0.5 transition-all duration-300 ${
      ...
      {isEmpty && (
        <div className="bg-white h-90 flex items-center justify-center text-center mx-auto rounded-3xl my-6">
          <div>
            ...
            <div className="py-6">
              <h1 className="text-[#121926] text-lg font-bold mb-2">
                No Moodboard Created Yet
              </h1>
